        ...
        tabItems( 
            ...
            tabItem(tabName = "insert_value", 
                fluidRow( 
                    ...
                    mainPanel( 
                        # Add Text 
                        textAreaInput(inputId = "Long_Text", label = "TEXT:", rows = 15, resize = "both"), 
                        br(), 
                        actionButton("update", "New Text"), 
                        br(), 
...
server <- function(input, output, session) { 
    ...
    output$d2 <- DT::renderDataTable({ 
        df = data.frame(
            QuoteDate = Sys.Date(), 
            Broker='<select Broker="" id="BrokerSelect"> <option value="RBC">RBC</option> <option value="BeechHill">BeechHill</option> </select>', 
            CUSIP="CUSIP", 
            Bond="Bond", 
            Size=0, 
            Actions='<select Actions="" id="ActionsSelect"> <option value="bid">bid</option> <option value="offer">offer</option> </select>', 
            Price=0
        ) 
        DT::datatable(df, escape=FALSE, selection = 'none', editable = 'cell', rownames=FALSE, options = list( 
            paging=FALSE, 
            searching=FALSE, 
            processing = FALSE, 
            initComplete = JS(js), 
            ...
            observeEvent(input$update, { 
                ...
                if (error != 1) { 
                    Query <- paste("BULK INSERT dbo.MarketQuotes FROM", dir_sql, "WITH (FORMAT = 'CSV', FIRSTROW=2, FIELDTERMINATOR =',', ROWTERMINATOR ='\\n');") 
            ...
            output$d3 <- DT::renderDT({
                RV_Aon$data
            })
            observeEvent(input$refresh, {
                RV_Aon$data <- sqlQuery(channel=db_conn, paste("SELECT * FROM dbo.Aon WHERE QuoteDate = (SELECT MAX (QuoteDate) FROM dbo.Aon)"), stringsAsFactors = FALSE)
            }, ignoreInit=T)

            observeEvent(input$refresh_Aon, {
                session$reload()
            })

            ### Function for BeechHill Weekly Table
            RV_BH <- reactiveValues(data = data.frame(BHQuote))
            output$d4 <- DT::renderDT({
                RV_BH$data
            })
            observeEvent(input$refresh, {
                RV_BH$data <- sqlQuery(channel=db_conn, paste("SELECT * FROM dbo.BH WHERE QuoteDate = (SELECT MAX (QuoteDate) FROM dbo.BH)"), stringsAsFactors = FALSE)
            }, ignoreInit=T)

            observeEvent(input$refresh_BH, {
                session$reload()
            })

            ### Function for GCS Weekly Table
            RV_GCS <- reactiveValues(data = data.frame(GCSQuote))
            output$d5 <- DT::renderDT({
                RV_GCS$data
            })
            observeEvent(input$refresh, {
                RV_GCS$data <- sqlQuery(channel=db_conn, paste("SELECT * FROM dbo.GCS WHERE QuoteDate = (SELECT MAX (QuoteDate) FROM dbo.GCS)"), stringsAsFactors = FALSE)
            }, ignoreInit=T)

            observeEvent(input$refresh_GCS, {
                session$reload()
            })

            ### Function for RBC Weekly Table
            RV_rbc <- reactiveValues(data = data.frame(RBCQuote))
            output$d6 <- DT::renderDT({
                RV_rbc$data
            })
            observeEvent(input$refresh, {
                RV_rbc$data <- sqlQuery(channel=db_conn, paste("SELECT * FROM dbo.RBC WHERE QuoteDate = (SELECT MAX (QuoteDate) FROM dbo.RBC)"), stringsAsFactors = FALSE)
            }, ignoreInit=T)

            observeEvent(input$refresh_rbc, {
                session$reload()
            })

            ### Function for SwissRe Weekly Table
            RV_swissre <- reactiveValues(data = data.frame(swissQuote))
            output$d7 <- DT::renderDT({
                RV_swissre$data
            })
            observeEvent(input$refresh, {
                RV_swissre$data <- sqlQuery(channel=db_conn, paste("SELECT * FROM dbo.SwissRe WHERE IndicationDate = (SELECT MAX(IndicationDate) FROM dbo.SwissRe)"), stringsAsFactors = FALSE)
            }, ignoreInit=T)

            observeEvent(input$refresh_swissre, {
                session$reload()
            })
